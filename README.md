# (仮称)タイムラインSNS

X(旧Twitter)風の、投稿・タイムライン・コメント・いいね・フォローができる学習目的のSNS
Webアプリケーション。受講生・個人など複数の利用者が使うことを想定し、いいね・コメントは
投稿者本人だけでなく第三者からも行える設計としている(詳細は[要件定義書](要件定義書.md)参照)。

現在、F-01〜F-15(認証・投稿の作成/編集/削除・全体/フォロー中タイムライン・コメント・いいね・
フォロー・プロフィール表示/編集・ユーザー検索)をすべて実装済み。AWSデプロイの構成
(Terraform)も用意している。

## ドキュメント

| ドキュメント | 内容 |
|---|---|
| [要件定義書](要件定義書.md) | 背景・目的・検収基準など全体の合意事項 |
| [機能一覧](docs/機能一覧.md) | 機能要件(F-01〜F-15)とスコープ外項目 |
| [画面設計書](docs/画面設計書.md) | 画面一覧(S-01〜S-06)・画面構成要素・画面遷移 |
| [データベース設計書](docs/データベース設計書.md) | ER図・テーブル定義・集計方針 |
| [技術スタック](docs/技術スタック.md) | 使用技術とバージョン |
| [ローカル開発環境セットアップ](docs/ローカル開発環境セットアップ.md) | 開発環境の構築手順・サーバー起動方法 |
| [インフラ構成](docs/インフラ構成.md) | AWS上のインフラ構成図・ディレクトリ構成(Terraform管理) |
| [ロギング設計](docs/ロギング設計.md) | 構造化ログのフィールド定義・ログレベル方針・運用監視/障害対応の考え方 |

## 機能一覧(概要)

詳細は[機能一覧](docs/機能一覧.md)を参照。

| 機能ID | 機能名 |
|---|---|
| F-01 | 新規登録 |
| F-02 | ログイン |
| F-03 | ログアウト |
| F-04 | 投稿の作成(テキスト+画像) |
| F-05 | 投稿の編集(自分の投稿のみ) |
| F-06 | 投稿の削除(自分の投稿のみ) |
| F-07 | 全体タイムライン表示 |
| F-08 | フォロー中タイムライン表示 |
| F-09 | コメントの投稿 |
| F-10 | コメント一覧・件数の表示 |
| F-11 | いいねの登録・解除 |
| F-12 | いいね数の表示 |
| F-13 | ユーザー検索(ユーザー名の部分一致) |
| F-14 | フォロー・フォロー解除 |
| F-15 | プロフィール画面表示(投稿一覧・フォロー数/フォロワー数) |

### X/Twitterとの差別化ポイント

- インプレッション数(閲覧数)は表示しない
- リツイート(拡散)機能は用意しない
- その代わり、フォロー機能により「全体」「フォロー中」の2種類のタイムラインを持つ

## 画面一覧

詳細・画面遷移・ワイヤーフレームは[画面設計書](docs/画面設計書.md)を参照。

| 画面ID | 画面名 |
|---|---|
| S-01 | ログイン画面 |
| S-02 | 新規登録画面 |
| S-03 | タイムライン画面(全体/フォロー中) |
| S-04 | 投稿詳細画面(コメント一覧) |
| S-05 | プロフィール画面 |
| S-06 | ユーザー検索画面 |

## データモデル概要

詳細・ER図は[データベース設計書](docs/データベース設計書.md)を参照。

- `users`: 利用者(ユーザー名・メールアドレス・パスワード・自己紹介)
- `posts`: 投稿(本文・画像URL(任意))
- `comments`: 投稿へのコメント
- `likes`: 投稿へのいいね(利用者×投稿でUNIQUE)
- `follows`: 利用者間のフォロー関係(自己参照の多対多)

いいね数・コメント数は`posts`にカウンタを持たず、`likes`/`comments`をその都度`COUNT`集計して
算出する方針としている。

## プロトタイプMock

技術スタックを決定する前に、要件定義書・機能一覧・画面設計書の内容(F-01〜F-15、S-01〜S-06)を
一通り操作して確認できる、HTML/CSS/JavaScriptのみの静的プロトタイプを用意している
(サーバー・データベースは使用せず、`mock/script.js`内のJavaScriptオブジェクトを疑似データベースとして
`localStorage`に保存しているだけの簡易実装)。

```bash
cd mock
python3 -m http.server 8899
# ブラウザで http://localhost:8899 を開く
```

デモ用アカウント(いずれもパスワード: `password`): `tanaka@example.com` / `sato@example.com` /
`suzuki@example.com` / `yamada@example.com`

## バックエンド(会員登録・ログイン・ログアウト)

Java(Spring Boot) + MyBatis + SQLite + JWT認証(アクセストークン+リフレッシュトークン方式)で
実装している。スキーマ管理はFlyway(`backend/src/main/resources/db/migration`)で行い、起動時に
バージョン管理されたマイグレーションを自動適用する。詳細は
[技術スタック](docs/技術スタック.md)・[ローカル開発環境セットアップ](docs/ローカル開発環境セットアップ.md)を参照。

```bash
cd backend
mvn spring-boot:run
# http://localhost:8080/api/auth/register 等にPOSTして動作確認できる
```

エンドポイント: `POST /api/auth/register`・`POST /api/auth/login`・`POST /api/auth/refresh`・
`POST /api/auth/logout`・`GET /api/auth/me`(認証必須)。アクセストークンは短命(15分)、
リフレッシュトークンは長命(7日)で、`/api/auth/refresh`を呼ぶたびにリフレッシュトークンも
ローテーションされる。ログアウト・リフレッシュで使用済みになったトークンはjti(トークンID)を
blacklistに登録することで、以降使えなくする。

## API仕様書(Swagger UI)

springdoc-openapiにより、コントローラのコードからOpenAPI仕様・Swagger UIを自動生成している。
ローカル実行時は`http://localhost:8080/swagger-ui.html`、本番(EC2)では
`http://<Elastic IP>/swagger-ui.html`でアクセスできる(認証不要で閲覧可能。ドキュメント自体の
公開範囲は、これまで`/uploads/**`を認証なしで配信していたのと同様の考え方)。OpenAPI(JSON)は
`/v3/api-docs`から取得できる。

保護されたエンドポイントは、`/api/auth/login`等で取得したアクセストークンを右上の「Authorize」
ボタンから貼り付けることで、Swagger UI上から直接試すことができる(`OpenApiConfig`で
`bearerAuth`セキュリティスキームを設定済み)。

## バックエンド(投稿の作成・編集・削除・タイムライン・コメント・いいね)

`POST /api/posts`(`multipart/form-data`。本文+画像は任意)・`GET /api/posts`(全体タイムライン、
新しい順)・`GET /api/posts/{id}`・`PUT /api/posts/{id}`・`DELETE /api/posts/{id}`(編集・削除は
本人の投稿のみ、他人の投稿への操作は403)。添付画像の保存先は`ImageStorageService`インターフェース
で抽象化しており、ローカル実行時は`backend/uploads/`にローカル保存して`/uploads/**`で静的配信(認証不要)、
本番(AWS)ではS3バケットに保存し、S3の公開URLをそのまま返す(いずれも認証不要でアクセス可能な点は同じ。
詳細は[インフラ構成](docs/インフラ構成.md)を参照)。

- `POST /api/posts/{id}/likes`: いいねのトグル(登録済みなら解除)。`{liked, likeCount}`を返す
- `GET /api/posts/{postId}/comments`・`POST /api/posts/{postId}/comments`: コメント一覧(新しい順)・投稿
- 投稿削除時は紐づくコメント・いいねも削除する

いいね数・コメント数・自分がいいね済みかは、投稿一覧取得時に投稿ごとの追加クエリを発行するのではなく、
`posts`取得と同じSQL内の相関サブクエリとして1回で集計している(N+1回避。詳細は
`backend/src/main/resources/mapper/PostMapper.xml`の`selectWithAuthor`を参照)。

`GET /api/posts`はカーソルベースのページネーションに対応しており、`cursor`(前回最後に取得した
投稿のid)・`limit`(デフォルト20、最大50)をクエリパラメータで指定できる。カーソルには
`created_at`ではなく`id`を使っている(`created_at`はミリ秒精度で一意性の保証がなく、同一ミリ秒に
複数投稿があると境界で重複/取りこぼしが起こり得るため。`id`は一意かつ投稿順と厳密に相関する)。
レスポンスは`{ items: [...], hasMore: boolean }`の形で返し、`limit+1`件取得して実際に超過していれば
`hasMore: true`とすることで次ページの有無を判定している。

## バックエンド(プロフィール・フォロー)

- `GET /api/users/{id}`: プロフィール取得(ユーザー名・自己紹介・フォロー数/フォロワー数・
  自分がフォロー済みか)。フォロー数等も`PostMapper`と同じ考え方で相関サブクエリとして
  1回のSQLで集計している(`UserMapper.xml`の`findProfileById`)
- `GET /api/users/{id}/posts`: 指定ユーザーの投稿一覧(新しい順)
- `PUT /api/users/me`: 自分のユーザー名・自己紹介・アイコン画像を更新(`multipart/form-data`。
  ユーザー名は他ユーザーと重複不可、自分自身の現在の値との比較は除外)。アイコン画像(`avatar`)は
  任意で、指定しない場合は既存の画像を保持する。保存先は投稿画像と同じ`ImageStorageService`
  (ローカル/S3)
- `POST /api/users/{id}/follow`: フォローのトグル(登録済みなら解除)。自分自身のフォローは400
- `GET /api/users?query=`: ユーザー名の部分一致検索(自分自身は除外)。空クエリなら自分以外の全ユーザーを返す

`GET /api/posts`は`?scope=following`を指定すると、フォロー中の利用者の投稿だけを返す
(`PostMapper`に`follows`とのJOINを追加しただけで、他の一覧取得と同じくN+1なしで実現している)。

## フロントエンド(新規登録・ログイン・タイムライン・投稿詳細・プロフィール・検索)

Vue.js(Vite) + TypeScriptで実装している。ログイン・新規登録に成功するとアクセストークン・
リフレッシュトークン・ユーザー情報を`localStorage`に保存し、タイムライン画面(`/timeline`)に
遷移する。

`src/lib/api.ts`の`authFetch()`が認証付きAPIリクエストの共通処理を担う。リクエストには
`Authorization: Bearer <アクセストークン>`ヘッダーを自動付与し、レスポンスが401(アクセス
トークン失効)の場合は`/api/auth/refresh`を1回呼んでトークンを再発行し、リクエストを
リトライする。それでも失敗する場合(リフレッシュトークンも失効)はログイン画面へ戻す。

`/timeline`画面では投稿の作成(本文+画像添付)・自分の投稿の編集(インライン)・削除・
いいねができ、「全体」「フォロー中」タブで投稿一覧の対象を切り替えられる(画面遷移せず
一覧内容のみ切り替わる)。投稿カードのコメント数をクリックすると投稿詳細画面(`/posts/:id`)に
遷移し、コメント一覧の閲覧・コメント投稿ができる。投稿カードは`src/components/PostCard.vue`
としてタイムライン・投稿詳細・プロフィールの3画面で共通利用している。

タイムラインは無限スクロールで追加読み込みする。一覧末尾に置いたセンチネル要素を
`IntersectionObserver`で監視し、画面に入ったら最後に表示している投稿のidを`cursor`として
次ページ(`GET /api/posts?cursor=...&limit=20`)を取得し、一覧に追記する。`hasMore: false`が
返るまで(または既に読み込み中なら)追加取得しない。

投稿者名やヘッダーの自分のユーザー名をクリックするとプロフィール画面(`/users/:id`)に遷移する。
自分のプロフィールでは「プロフィールを編集」からユーザー名・自己紹介・アイコン画像を変更でき、他人の
プロフィールでは「フォローする」/「フォロー解除」ボタンを表示する。ヘッダーの検索アイコンから
ユーザー検索画面(`/search`)に遷移し、ユーザー名で絞り込み・各結果からのフォローができる。

```bash
cd frontend
npm install
npm run dev
# http://localhost:5173 を開く(/api・/uploadsは自動的にバックエンド(8080)へプロキシされる)
```

## バックエンドのテスト

```bash
cd backend
mvn test
```

JUnit 5で3層に分けてテストしている。

- **サービス層の単体テスト**(`src/test/java/com/snsapp/service/`): Mockitoでマッパー等をモック化し、
  Springコンテキストを起動せず高速に検証する
- **MyBatisマッパーのDB統合テスト**(`src/test/java/com/snsapp/mapper/`): `@MybatisTest`を使い、
  一時的なインメモリSQLite(`file::memory:?cache=shared`。`src/test/resources/application-test.properties`)
  に対して実際にSQLを実行して検証する。本番・ローカル開発用のDB(`sns.db`)には一切触れず、
  プロセス終了時に自動的に破棄される。schema.sql・MyBatisのXMLは本番と完全に同じものを使うため
  (H2等への置き換えはしていない)、本番と同じSQL方言で実際に動くかを正確に検証できる
- **コントローラーの統合テスト**(`src/test/java/com/snsapp/controller/`): `@SpringBootTest` +
  `MockMvc`で、JWT認証フィルタも含めた実際のHTTPリクエスト〜レスポンスを検証する
  (これまで手動でcurl確認していた内容の自動化)

いずれのテストも`@Transactional`により、テストメソッドごとに書き込んだデータを自動的に
ロールバックしてから次のテストに進む。

## フロントエンドのテスト

```bash
cd frontend
npm test
```

Vitest + Vue Test Utils(jsdom環境)でテストしている。E2Eテストは対象外。

- **`src/lib/__tests__/`**: `api.ts`の単体テスト。`fetch`をモック化し、認証ヘッダーの付与、
  アクセストークン失効時(401)のリフレッシュ→リトライ、エラーメッセージの解析などを検証する。
  `format.ts`の日時フォーマットも検証する
- **`src/components/__tests__/`・`src/views/__tests__/`**: 各UIコンポーネント・画面のテスト
  (`PostCard`・`LoginView`・`RegisterView`・`TimelineView`・`ProfileView`・`SearchView`・
  `PostDetailView`)。`vue-router`・`src/lib/api.ts`をモック化し、表示内容・フォーム送信時の
  API呼び出し・エラー表示・認証失効時のログイン画面へのリダイレクトなどを検証する。
  `@testing-library/vue`を使い、CSSセレクタではなく`getByRole`・`getByLabelText`等の
  ラベル/ロールベースのクエリで要素を取得している(アクセシブルネームが絵文字+数字のみになる
  いいねボタン・コメント数リンクには、そのために`PostCard.vue`側へ`aria-label`を付与している)

## ディレクトリ構成

```
.
├── 要件定義書.md
├── docs/            # 各種設計ドキュメント
├── mock/            # HTML/CSS/JSのみのプロトタイプMock(DB・サーバーなし)
├── backend/         # Spring Boot(MyBatis, SQLite, JWT認証)バックエンド + Dockerfile
├── frontend/        # Vue.js(Vite)フロントエンド + Dockerfile・nginx.conf
├── terraform/       # AWSインフラのコード化(EC2・ネットワーク・IAM・S3。RDSはなし)
├── scripts/deploy.sh # SSM経由の再デプロイスクリプト
├── docker-compose.prod.yml # 本番環境でのコンテナ起動定義
└── .claude/skills/  # 開発支援スキル(タスクボードプロジェクトから流用)
```

## 実装状況

- [x] リポジトリ作成・要件定義ドキュメント一式(要件定義書・機能一覧・画面設計書・データベース設計書)
- [x] HTML/CSS/JSプロトタイプMock
- [x] 会員登録・ログイン・ログアウトのバックエンド実装(F-01〜F-03、アクセス+リフレッシュトークン方式)
- [x] 新規登録・ログイン画面のフロントエンド実装
- [x] 投稿の作成・編集・削除、全体タイムライン表示(F-04〜F-07)
- [x] コメント・いいね(F-09〜F-12)
- [x] フォロー・フォロー解除、プロフィール表示/編集(F-14, F-15)
- [x] フォロー中タイムライン(F-08)・ユーザー検索(F-13)
- [x] AWSデプロイ構成(Terraform、EC2 + SSM。RDSは使わずSQLiteをEC2ローカルで永続化)
- [x] 投稿画像の保存先をS3に移行(EC2ローカル保存から切り替え。IAMインスタンスプロファイル経由でアクセス)
- [x] プロフィール編集でアイコン画像を設定可能に(投稿画像と同じ`ImageStorageService`経由で保存)
- [x] springdoc-openapiによるSwagger UI / OpenAPI仕様書の自動生成
- [x] バックエンドのテスト整備(サービス単体・MyBatisマッパーDB統合・コントローラー統合、計116件)
- [x] フロントエンドのテスト整備(Vitest + Vue Test Utils、単体・コンポーネントテスト計65件)
- [x] バックエンドに構造化ログ(JSON)を導入([ロギング設計](docs/ロギング設計.md)参照)
- [x] DBスキーマ管理をFlywayによるバージョン管理されたマイグレーションに移行(手書きの起動時ALTER処理を廃止)
- [x] フロントエンドをTypeScriptに全面移行(`vue-tsc`による型チェックを`npm run build`に統合)
- [x] タイムラインにカーソル(id)ベースのページネーションと無限スクロールを実装

## 開発ワークフロー

Issue登録・ブランチ運用・Pull Requestのルールは[CLAUDE.md](CLAUDE.md)を参照。
